import time
import pytest
from agentic_core.application.services.behavioral_monitor import BehavioralMonitor, AgentBehaviorProfile
from agentic_core.application.services.compliance import ComplianceService, ComplianceConfig, ComplianceEvent, RiskClassifier, RiskLevel
from agentic_core.application.services.cost_enforcer import CostEnforcer, CostBudget, BudgetAction
...
class TestCompliance:
    ...
    def test_enabled_file_store(self, tmp_path):
    ...
    def test_retention_check(self, tmp_path):
        cfg = ComplianceConfig(enabled=True, storage_backend="file", storage_path=str(tmp_path))
        svc = ComplianceService(cfg)
        svc.record(ComplianceEvent(event_type="test"))
